...
//в файле swilib.h структуру NativeExplorerData заменить на ниже приведенную
/*
typedef struct
{
...
/*0x  */ WSHDR ws2;
/*0x  */ WSHDR ws3;
/*0x  */ unsigned short ws1_body[140];
/*0x  */ unsigned short ws2_body[140];
/*0x  */ unsigned short ws3_body[140];
} MAIN_CSM;
  ...
  if (msg->msg==MSG_CSM_DESTROYED && (int)msg->data0==csm->EXP_CSM_ID)  
   {
     if(csm->expl.full_filename->wsbody[0]!=0)
     {
       ...
       if(reslen==0)
       {
         ...
         j_SettingsUpdate(0x302, entry);//обновили кэш?
       }
       j_sprintf(csm->appid_new, "XTRA_JC%08x", crc);
       j_GBS_SendMessage(0x4209, 0x1B6, 1, csm->appid_new, csm->appid_cur);//для IDLE hotkey
       j_GBS_SendMessage(0x4209, 0x1B8, 1, csm->appid_new, csm->appid_cur);//для "Мое меню"
       csm->appid_new=0;
       csm->unk50=1;
       csm->csm.state=-3;
     }
