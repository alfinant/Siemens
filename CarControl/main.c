...
void tmr_call_proc()
{
  ...
  if(*RamCallState()==3)// 1-исходящий вызов;2-исходящий вызов,идут гудки;3-входящий вызов;4-трубка взята
  {
...
void InitPath()
{
  ...
  sprintf(sndLock, "%s%s", DIR, "Sounds\\lock.wav");
  
  sndUnlock=malloc(strlen(DIR)+32);
  sprintf(sndUnlock, "%s%s", DIR, "Sounds\\unlock.wav"); 
}
...
{
  wsprintf((WSHDR *)(&MAINCSM.maincsm_name),"CarControl");
}
