...
typedef struct
{
   __IO uint32_t CLK;      /*!< Measurement Peripheral Clock Control Register */
   __IO uint32_t RSV1;
   __IO uint32_t ID;
   __IO uint32_t RSV2;
   __IO uint32_t UNK1;
   ...
   __IO uint32_t DATA7;
   __IO uint32_t UNK2;
   __IO uint32_t RCV3[43];
   __IO uint32_t RDYIRQ;
   __IO uint32_t INT_CTRL2;
     
} MEAS_TypeDef;                   

/*!< Peripheral_declaration */  

__no_init MEAS_TypeDef MEAS  @ ((uint32_t)0xF4C00000); 

//*************************** MEAS.CTRL2 **************************************/
#define MEAS_ENTRIG         (uint32_t)(1 << 28)    /*!< Triggered Mode Enable */
#define MEAS_ADCON          (uint32_t)(1 << 29)   /*!< ADC Power Save Control */

//*************************** MEAS.STAT ***************************************/
#define MEAS_BUSY            (uint32_t)(1 << 30)               /*!< Busy Flag */
#define MEAS_READY           (uint32_t)(1 << 31)              /*!< Data ready */
...
__no_init USART_TypeDef S0  @ ((uint32_t)0xF1000000);    
__no_init USART_TypeDef S1  @ ((uint32_t)0xF1800000);
...
typedef struct
{
   ...
   __IO uint32_t USART0_RXD;             //0xF430004C         /*-->AC_RX, 4й пин на коннекторе */
   __IO uint32_t USART0_TXD;             //0xF4300050         /*-->AC_TX, 3й пин на коннекторе, +подт€гивающий резистор, ALT3(CCU0->CC2) */
   __IO uint32_t USART0_RTS;             //0xF4300054         /*-->AC_RTS, 6й пин на коннекторе, +подт€гивающий резистор, ALT3(CCU0->CC6) */   
   __IO uint32_t USART0_CTS;             //0xF4300058         /*-->AC_CTS, 5й пин на коннекторе, +подт€гивающий резистор, ALT3(CCU1->CC2) */
   __IO uint32_t DSPOUT0;                //0xF430005C         /*-->AC_DCD, 7й пин на коннекторе, +подт€гивающий резистор, ALT3(CCU1->CC6) */
   __IO uint32_t USART1_RXD;             //0xF4300060 
   __IO uint32_t USART1_TXD;             //0xF4300064           
   __IO uint32_t USART1_RTS;             //0xF4300068       
   __IO uint32_t USART1_CTS;             //0xF430006C          
   __IO uint32_t USB_DPLUS;              //0xF4300070    
   __IO uint32_t USB_DMINUS;             //0xF4300074    
   __IO uint32_t UNK78;                  //0xF4300078 
   ...
   __IO uint32_t CC_VZ;                  //0xF4300098         /*-->PM_RINGIN */  
   __IO uint32_t I2S_UNK0;               //0xF430009C         I2S2, отвечают за звук  требуетс€ уточнить!!!
...
__no_init GPIO_TypeDef GPIO  @ ((uint32_t)0xF4300000);

/*******************  Bit Mask for GPIO registers  **********************/ 
...
#define  GPIO_PDPU                    (3 << 13)                                 /*! [14:13] (Pullup/Pulldown Selection) ¬кл/¬ыкл внутреннего подт€гивающего резистра: 00-без подт€щки ,01-Pullup(подт€гивающий резистор подключен к плюсу), 02-Pulldown(подт€гивающий резистор подключен к земле) */                     
#define  GPIO_ENAQ                    (1 << 15)                                 /*! [15] 0-—осто€ние вывода определ€етс€ другими битами, 1-¬ывод сброшен,низкий уровень */
...
__no_init CAPCOM_TypeDef CCU0  @ ((uint32_t)0xF4000000); 
__no_init CAPCOM_TypeDef CCU1  @ ((uint32_t)0xF4100000); 
