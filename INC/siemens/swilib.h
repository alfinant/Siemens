#ifndef HEADER_SWILIB_H
#define HEADER_SWILIB_H
...
#define UTF16_FONT_LARGE (0xE018)
...
// For kbd msg:
// submsg - scan Code:            //msg->keys
#ifdef NEWSGOLD
#define LEFT_SOFT       0x01      // 0x0018
#define RIGHT_SOFT      0x04      // 0x0001
#define GREEN_BUTTON    0x0B      // 0x0005
#define RED_BUTTON      0x0C      // 0x..?
#define VOL_UP_BUTTON   0x0D      // 0x000F
#define VOL_DOWN_BUTTON 0x0E      // 0x000E
#define UP_BUTTON       0x3B      // 0x0026
#define DOWN_BUTTON     0x3C      // 0x0025
#define LEFT_BUTTON     0x3D      // 0x0028
#define RIGHT_BUTTON    0x3E      // 0x0027
#define ENTER_BUTTON	0x1A      // 0x003D
#ifdef ELKA                       // '*' 0x0014
#define POC_BUTTON 0x15           // '#' 0x0015
#define MEDIA_BUTTON	0x11      // '0-9' 0x0016
...
typedef struct
{
  ...
  void (*ghook)( void* gui, int cmd); //GUI * gui, int cmd
  void *proc3;
...
//  0x80000000
  ...
  void (*itemproc)(void *gui,int curitem, void *); //Called when draw item
...
typedef struct
{
  int flag; //0,8 etc
  int (*onkey)( void *gui, GUI_MSG *msg);
  void (*ghook)( void * gui, int cmd);
  void *proc3;
  const int *softkeys; //{6,0x22,0x1D}, mb default for all items, if item.softkeys==NULL
  const SOFTKEYSTAB *softkeystab;
  int flags2; //2
  void (*itemproc)( void *gui, int curitem, void *user_pointer); //Called when draw item
  const MENUITEM_DESC *items; //Table of items desc;
  const MENUPROCS_DESC *procs;//  void ** procs; //Table of procs when item selected
  int n_items; //Number of items
...
#pragma swi_number=0x39
__swi __arm int Registry_SetResourcePath(int keyID, int prio, WSHDR *path);

#pragma swi_number=0x3A
__swi __arm int Registry_DeleteResourcePath(int keyID, int prio);

#pragma swi_number=0x3E
__swi __arm int Registry_GetResourcePathFromLevel(int keyID, int startPrio, int *pbIsPrio, WSHDR *path);
...
__swi __arm void StartMidlet(char*,int);
...
#pragma swi_number=0x65
__swi	__arm void EndCall(void);

#pragma swi_number=0x66
__swi	__arm void EndCallBusy(int param1, int param2);

#pragma swi_number=0x6E
__swi	__arm int IsCalling(void);

#pragma swi_number=0x6F
...
__swi __arm int DrawObject2Layer(LCDLAYER *, DRWOBJ *);
...
#pragma swi_number=0x80BA
__swi __arm void *realloc_adr(void);
...
__swi __arm int divide(unsigned int denom,unsigned int number);
//arm
...
__swi __arm void *RAM_LCD_Layer_Base_Ptr();
...
__swi __arm int GBS_ReceiveMessage(GBS_MSG *);
//arm
...
#pragma swi_number=0x058
__swi	__arm	void ClearMemory (void *dest,int n);
...
__swi __arm int async_gethostbyname(const char *host_name, HOSTENT **,int *DNR_ID);
//thumb
...
__swi __arm BIGNUM *BN_bin2bn(const unsigned char *s,int len,BIGNUM *ret);
//thumb
...
__swi __arm int OpenExplorer(NativeExplorerData*);
...
#pragma swi_number=0x8253
__swi __arm char *RamLastCallNum();
...
__swi __arm int Obs_SetInputFile (HObj hObj, int unk_zero, WSHDR *path);
//arm
...
__swi __arm int Obs_SetInputMemory(HObj obj, int unk, char *buf, int len);
//pattern_NSG/ELKA=F0412DE90380A0E10270A0E10060A0E10150A0E10040A0E3??????EB000050E3

#pragma swi_number=0x27B
__swi __arm int Obs_SetCSM(HObj obj, CSM_RAM *csm);
//pattern_NSG/ELKA=70402DE90160A0E10050A0E10040A0E3??????EB000050E30540A003??????0A0610A0E10500A0E1
...
__swi __arm unsigned int OpenNativeAddressbook(int type, int unk1, int unk2, void *data); //type, 0x4, select num,
...
//arm
//pattern_SGOLD=30402DE90040A0E18000A0E3


#pragma swi_number=0x2E0
__swi __arm IMGHDR *GetIMGHDRFromThemeCache(int id);
//thumb
//pattern=01,1C,??,??,1C,30,??,29,??,??,??,??,5B,5C,5B,00,9F,44

#pragma swi_number=0x2E1
__swi __arm int GetExtUidByFileName(WSHDR *fname);
//arm
...
//arm
//pattern_ELKA=04,E0,2D,E5,24,D0,4D,E2,20,20,8D,E2,04,10,8D,E2
//pattern_NSG=80,B5,88,B0,07,AA,69,46,??,??,??,EA,00,28,+1
...
//thumb
//pattern_NSG/ELKA=F0,B5,89,B0,05,1C,0E,20,00,AB,18,77,0C,20,68,43,75,49,44,18,+1
...
__swi __arm LCDLAYER *GetLCDLAYERByCurCepID();

#pragma swi_number=0x8394
__swi __arm RAND_METHOD *RAND_get_rand_method(void);
//pattern_NSG/ELKA=52 41 4E 44 20 70 61 72 74 20 6F 66 20 4F 70 65 6E 53 53 4C 20 30 2E 39 2E 35 61 20 31 20 41 70 72 20 32 30 30 30 00 00 +0x28

#pragma swi_number=0x395
__swi __arm SSL_METHOD *TLSv1_client_method();
//pattern_NSG/ELKA=10,B5,??,??,??,??,??,??,01,1C,64,22,64,34,20,1C,??,??,??,??,??,??,60,61 +1

#pragma swi_number=0x396
__swi __arm SSL_CTX *SSL_CTX_new(SSL_METHOD *meth);
//pattern_NSG/ELKA=02,20,A0,62,00,26,25,60,05,20,00,03,66,61,E0,61,26,62,66,62,28,6D,80,47 -0x25

#pragma swi_number=0x397
__swi __arm void SSL_CTX_free(SSL_CTX *ctx);
//pattern_NSG/ELKA=38,B5,04,1C,??,??,34,4A,00,21,00,92,0C,22,C9,43,20,1C,68,30 +1

#pragma swi_number=0x398
__swi __arm long SSL_CTX_ctrl(SSL_CTX *ctx, int cmd, long larg, void *parg);
//pattern_NSG/ELKA=B0,B5,0D,1C,04,1C,18,1C,14,3D,1A,2D +1

#pragma swi_number=0x399
__swi __arm SSL *SSL_new(SSL_CTX *ctx);
//pattern_NSG/ELKA=F8,B5,05,1C,??,??,B8,23,??,??,??,??,??,??,28,68,00,28 +1

#pragma swi_number=0x39A
__swi __arm int SSL_set_fd(SSL *s, int fd);
//pattern_NSG/ELKA=F8,B5,07,1C,0E,1C,00,25,??,??,??,??,??,??,??,??,04,1C,??,??,FF,23,B9,33 +1

#pragma swi_number=0x39B
__swi __arm int SSL_connect(SSL *ssl);
//pattern_NSG/ELKA=10,B5,04,1C,00,6A,00,28,??,??,20,1C,??,??,??,??,A0,68 +1

#pragma swi_number=0x39C
__swi __arm int SSL_shutdown(SSL *ssl);
//pattern_NSG/ELKA=80,B5,01,6A,00,29,??,??,??,4B,??,4A +1

#pragma swi_number=0x39D
__swi __arm int SSL_read(SSL *ssl, void *buf, int num);
//pattern_NSG/ELKA=F8,B5,0F,1C,04,1C,16,1C,??,??,??,??,??,??,??,??,00,21,01,60 +1

#pragma swi_number=0x39E
__swi __arm int SSL_peek(SSL *ssl, void *buf, int num);
//pattern_NSG/ELKA=F7,B5,05,1C,17,1C,??,??,??,??,6C,6D,B8,34,60,68,00,28 +1

#pragma swi_number=0x39F
__swi __arm int SSL_write(SSL *ssl, const void *buf, int num);
//pattern_NSG/ELKA=F8,B5,0E,1C,04,1C,15,1C,??,??,??,??,??,??,??,??,00,27,07,60 +1

#pragma swi_number=0x3A0
__swi __arm void SSL_free(SSL *ssl);
//pattern_NSG/ELKA=38,B5,04,1C,??,??,FF,22,4D,32,00,92,00,21,C9,43,10,22,20,1C +1

#pragma swi_number=0x3A1
__swi __arm int SSL_set_session(SSL *ssl, SSL_SESSION *session);
//pattern_NSG/ELKA=F8,B5,07,1C,06,1C,80,36,C0,37,0D,1C,04,1C,00,29,??,??,F9,68,28,68 +1

#pragma swi_number=0x3A2
__swi __arm void SSL_SESSION_free(SSL_SESSION *sess);
//pattern_NSG/ELKA=38,B5,04,1C,??,??,FF,22,DA,32,00,92,00,21,C9,43,0E,22,20,1C +1

#pragma swi_number=0x3A3
__swi __arm void SetMenuItemIconIMGHDR(void *gui, void *item, IMGHDR *img);
//pattern_NSG/ELKA=00,2A,??,??,0A,64,C8,6B,80,22,10,43,C8,63,70,47 +1

#pragma swi_number=0x3A4
__swi __arm void SetDrawingCanvas(int x, int y, int x2, int y2);
//pattern_NSG/ELKA=46,63,74,5F,30,31,00,00,??,??,??,A0,20,B7,00,00,46,63,74,5F,30,32,00 -0x3CF

#pragma swi_number=0x3A5
__swi __arm void DrawIMGHDR(int x, int y, IMGHDR *img);
//pattern_NSG/ELKA=FF,B5,81,B0,04,1C,05,20 +0x157

#pragma swi_number=0x3A6
__swi __arm void Get_Obj1_WH(DRWOBJ *drwobj, int *w, int *h);
//pattern_NSG/ELKA=F0,B5,0E,1C,15,1C,04,1C,00,28,??,B0,??,??,20,69 +1

#pragma swi_number=0x3A7
__swi __arm int GUI_NewTimer(GUI *gui);
//thumb
//pattern_NSG/ELKA=F8,B5,05,1C,??,??,00,24,04,38,01,68,E4,43,00,29 +1

#pragma swi_number=0x3A8
__swi __arm int GUI_DeleteTimer(GUI *gui, int id);
//thumb
//pattern_NSG/ELKA=F3,B5,??,??,0C,1C,04,3E,30,68,81,B0,00,28 +1

#pragma swi_number=0x3A9
__swi __arm void GUI_StartTimerProc(GUI *gui, int id, long ms, void ptr());
//thumb
//pattern_NSG/ELKA=F8,B5,04,1C,10,1C,1A,1C,0A,29,??,??,14,23,59,43 +1

#pragma swi_number=0x3AA
__swi __arm void qsort(void *first, size_t number, size_t size, int (*comparator)(const void*, const void*));
//thumb
//pattern=F0,B5,0F,B4,07,1C,0E,1C,14,1C,81,B0,??,??,0A,2E +1

#pragma swi_number=0x3AB
__swi __arm void *bsearch(const void *searchkey, const void *baseptr, size_t number, size_t size, int (*funccompar)(const void*, const void*));
//thumb
//pattern=F0,B5,0F,B4,0F,1C,14,1C,81,B0,00,2C,??,??,00,20,05,B0,F0,BC +1


//#pragma swi_number=0x3AC
//__swi __arm int EVP_add_cipher(EVP_CIPHER *c);
//thumb
//pattern=10,B5,04,1C,00,68,??,??,??,??,22 1C 02 21 +1
