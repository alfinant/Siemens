...
const int Repl_chars_count = 6;
REPL_ARRAY Repl_chars[] = {"&apos;\0",0x27,
                           "&#39;\0\0",0x27,//added 25.09.2017 by alfinant
                           "&quot;\0",'"',
                           "&lt;\0\0\0", '<',
                           "&gt;\0\0\0", '>',
                           "&amp;\0\0", '&',
...
//************************ added by alfinant ***********************************
//вычисляет размер памяти занимаемый символом utf8 
static int UTF8_GetSize(char *uchar)
{
  int n=0;
  
  if (*uchar < 0x80) n=1;
  else
    if ((*uchar & 0xF8) == 0xF0) n=4;
    else
      if ((*uchar & 0xF0) == 0xE0) n=3;
      else
        if ((*uchar & 0xE0) == 0xC0) n=2;
      //else
      //if (*uchar == 0) n=0;
      return n;  
}

static int DecodeUTF8(const char *bytes, unsigned *ch)
{
  int n;
  ...
  if ((*bytes & 0xF8) == 0xF0) n=4;
  else
    if ((*bytes & 0xF0) == 0xE0) n=3;
  else
    if ((*bytes & 0xE0) == 0xC0) n=2;
  else
    return(0);
  
  *ch= *bytes & ((0x07 << (4 - n)) | 0x03);
...
static int EncodeUTF8(unsigned ch, int len, char *bytes)
{
  if (ch <= 0x7F)
  {
    if (len >=1)
    ...
    if (ch <= 0xFFFF)
    {
      if (len >=3)
      {
        *bytes++ = 0xE0 | (0x0F & (ch >> 12));
        *bytes++ = 0x80 | (0x3F & (ch >> 6));
        *bytes = 0x80 | (0x3F & ch);
        return(3);
      }
    }
    ...
    if (ch <= 0x10FFFF)
    {
      if (len >=4)
      {
        *bytes++ = 0xF0 | (0x07 & (ch >> 18));
        *bytes++ = 0x80 | (0x3F & (ch >> 12));
        *bytes++ = 0x80 | (0x3F & (ch >> 6));
        *bytes = 0x80 | (0x3F & ch);
        return(4);
      }
...
//получить кол-во символов в utf8 строке
int utf8_syms_n(char* utf8_str, int len)
{
  int n=0;
  char* s = utf8_str;  
  
  while (s < (utf8_str + len))
  {
   s+=UTF8_GetSize(s);
   n++;
  }
...
int Replace_Smiles_Syms(char* utf8_str)
{//адаптирует utf-8 стороку под WSHDR, если там есть смайлы и символы выше 0xFFFF 
  unsigned ch;
  ...
  while (b < (utf8_str + len))
  {
    wchar=0xFFFF;
    ch=0;
    
    switch(UTF8_GetSize(b))
    {
    case 0:
      return(0);
    ...
    case 3:
      ...
      {
        if (ch==0x2639) wchar=0xE200+113;
        if (ch==0x263A) wchar=0xE200+114;
        if (ch==0x270C) wchar=0xE200+115;
        if (ch==0x2764) wchar=0xE200+116;
        
        int i= EncodeUTF8(wchar, 4, u);
        n+=i;
        u+=i;
        b+=3;
      }
      else
        if (ch >= 0xFE00 && ch <= 0xFE0F)//селекторы вариантов начертания-игнор
          b+=3;
        else
        {
          *u++ = *b++;
    ...
    case 4:
      b += DecodeUTF8(b, &ch);
      if (ch >= 0x1F600 && ch < 0x1F650)//emoji
        wchar=0xE200 + (ch - 0x1F600) + 3;
      else
        if (ch >= 0x1F440 && ch <= 0x1F450)
          wchar=0xE200 + ch - 0x1F440 + 96;      
        else
          if (ch >= 0x1F3FB && ch <= 0x1F3FF)//модификаторы эмоджи-игнор
            break;
        int i= EncodeUTF8(wchar, 4, u);
        n+=i;
        u+=i;
...
WSHDR * CreateWS_emoji(char *utf8_str)//создает WSHDR строку с поддержкой смайлов(некоторых).Не работает без dyn_images.c
{
  if (utf8_str == 0 || *utf8_str == 0)//Если указатель нулевой или строка пуста
    return AllocWS(4);

  int len = strlen(utf8_str);
  char* __utf8_str = malloc(len + 1);
  strcpy(__utf8_str, utf8_str);
  len = Replace_Smiles_Syms(__utf8_str);
  WSHDR *ws = AllocWS(utf8_syms_n(__utf8_str, len));
  utf8_2ws(ws, __utf8_str, len);
  mfree(__utf8_str);
