...
{$ENDIF}
{$IFDEF BFBMODE}
function AddBFBEEPBlk(num:dword; flgFind:boolean): boolean;
{$ENDIF}
function AddEEPBlk(num:dword; ver : byte; len: integer; var buf : array of byte): boolean;
...
begin
        ...
        if Not BFB_EE_Get_Block_Info(num,len,ver) then begin
         if BFB_Error=ERR_BFB_EEPBLKNONE then begin
          if flgFind then result:=True
          else sEep_Err:='Нет или неоткрыт блок '+IntToStr(num)+'!';
          exit;
         end
         else if Not BFB_EE_Get_Block_Info(num,len,ver) then begin
        ...
        if (len > MAX_SIZE_EEPBLK) then begin
         ...
         if len<>0 then begin
          if Not BFB_EE_Read_Block(num,0,len,EepBuf[idx_eepbuf]) then begin
           if Not BFB_EE_Read_Block(num,0,len,EepBuf[idx_eepbuf]) then begin
            if Not BFB_EE_Read_Block(num,0,len,EepBuf[idx_eepbuf]) then begin
             sEep_Err:='Ошибка чтения блока '+IntToStr(num)+'!';
             exit;
...
begin
         ...
         if len<>0 then move(buf[0],EepBuf[idx_eepbuf],len);
         EepTab[idx_eeptab].num:=num;
         EepTab[idx_eeptab].len:=len;
         EepTab[idx_eeptab].ver:=ver;
         EepTab[idx_eeptab].addr:=0;
